...
searchButton.addEventListener("click", async () => {
  ...
  try {
    ...
    outputTag.innerHTML = `<span>${feedbackData.name}</span> is from <span>${feedbackData.country[0].country_id}</span> with <span>${Math.floor(feedbackData.country[0].probability * 100)}</span>% certainity.`;
  } catch (error) {
    outputTag.textContent = "Name not defined..";
  } finally {
